...
In perturbation theory, it is the standard approach (although there are some exceptions, see e.g. \cite{Parisi:1980ys}) to reinforce a gauge-fixing procedure: this introduces new gluon-ghost vertices
...
\eqref{eq: Z Wilson} and write the integration measure explicitly as $[D U] = \prod_{\tau=0}^{N_T-1} [D U_0(\tau)] [D U_i(\tau)]$; then it follows that
\begin{align}
...
We can also recover this relation the other way around. Let's expand the trace of $\hat{T}^{N_T}$ over a complete set of states, for instance
\begin{equation}
